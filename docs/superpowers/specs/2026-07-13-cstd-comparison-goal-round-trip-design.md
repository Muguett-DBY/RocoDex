...
This is an interaction provenance problem: ordinary goal browsing should stay in the guide, while a visitor sent there from a comparison should return to the same comparison after making the requested choice.
...
- Return only comparison-originated goal selection to `#project-comparison` after a non-null goal is selected.
...
The comparison action records a transient interaction handoff, serializes `#project-guide`, and focuses the guide heading. A subsequent non-null goal selection preserves the comparison query, serializes `#project-comparison`, and focuses the updated result. Normal guide selection remains unchanged.
...
4. Clear the transient guide-handoff marker and mark one interaction-driven comparison focus transfer.
...
Goal choices reached from navigation, directory browsing, or a restored guide URL keep the existing `#projects` behavior and retain focus on the selected goal button. No automatic comparison return occurs.
...
- Add transient comparison-to-guide handoff state in `CstdLanding`. It is render state because it drives a post-render guide alignment/focus effect and is intentionally not serialized.
...
- **Stale provenance:** clear the handoff as soon as a goal is selected and derive return behavior only from the explicit outbound action.
...
- **Ordinary-flow regression:** the special return hash is gated by transient comparison provenance, leaving all other goal selection calls on their current default path.
