...
describe("auth page runtime configuration", () => {
  it("keeps login and register layouts dynamic so runtime auth env is not frozen at build time", () => {
    const registerLayout = readFileSync(join(process.cwd(), "src", "app", "(rocodex)", "register", "layout.tsx"), "utf8");
    const loginLayout = readFileSync(join(process.cwd(), "src", "app", "(rocodex)", "login", "layout.tsx"), "utf8");
