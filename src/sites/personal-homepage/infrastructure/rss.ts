...
export function serializeCstdRss(locale: CstdLocale) {
  ...
  const link = createCstdUrl(locale === "zh" ? "/notes" : "/en/notes");
  const notes = [...cstdTechnicalNotes].sort((left, right) => right.updatedAt.localeCompare(left.updatedAt));
  ...
  const latestUpdate = notes[0]?.updatedAt ?? "1970-01-01";
