  ...
  return (
    <ol data-cstd-capability-timeline className="border-t border-white/15">
      {entries.map((entry) => {
        ...
        const content = (
          <>
            <div className="flex items-center gap-3"><Icon aria-hidden="true" className="h-4 w-4 text-[#24e0ff]" /><span className="font-mono text-[11px] font-black text-[#738086]">{entry.kind.toUpperCase()}</span></div>
            ...
            <div className="flex items-start justify-between gap-4 font-mono text-[11px] font-black text-[#f4d431]"><span>{entry.date}</span><ArrowUpRight aria-hidden="true" className="h-4 w-4 transition-transform group-hover:-translate-y-1 group-hover:translate-x-1" /></div>
          </>
        );
