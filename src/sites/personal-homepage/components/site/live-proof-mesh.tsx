    ...
    <section data-cstd-proof-mesh data-cstd-proof-mesh-size={caseSlug ? "case" : "global"} aria-labelledby={`proof-mesh-${caseSlug ?? "global"}`} className="border-y border-white/12 bg-[#050709]/92 px-5 py-10 text-[#f2efe7] backdrop-blur-xl md:px-10 lg:px-16">
      <div className="mx-auto max-w-[1540px]">
        <header className="flex flex-wrap items-end justify-between gap-6">
          <div>
            <p className="flex items-center gap-2 font-mono text-[9px] font-black text-[#3dff8f]"><RadioTower aria-hidden="true" className="h-4 w-4" /> LIVE PROOF MESH / BUILD-TIME</p>
            ...
            return (
              <CstdLink key={entry.id} href={locale === "en" ? `/en/work/${entry.caseSlug}` : `/work/${entry.caseSlug}`} data-cstd-proof-node={entry.caseSlug} className="group min-w-0 border-b border-r border-white/12 bg-white/[0.018] p-5 transition-colors hover:bg-white/[0.05] focus-visible:outline focus-visible:outline-2 focus-visible:outline-inset focus-visible:outline-[#24e0ff]">
                <div className="flex items-start justify-between gap-4">
                  <span className="inline-flex items-center gap-2 font-mono text-[8px] font-black" style={{ color: accent }}><CheckCircle2 aria-hidden="true" className="h-3.5 w-3.5" />{entry.status.toUpperCase()} / {entry.coverageScore}</span>
                ...
                <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 font-mono text-[8px] font-black text-[#778388]"><span>{entry.artifactCount} ARTIFACTS</span><span>{entry.artifactKinds.length} TYPES</span><span data-cstd-proof-freshness={freshness}>{freshnessCopy[locale][freshness]} / {entry.verifiedAt}</span></div>
              </CstdLink>
            );
          })}
        </div>
