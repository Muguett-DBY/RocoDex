...
export function ExecutableCaseReplay({ replay, locale, compact = false }: { replay: CstdCaseReplay; locale: CstdLocale; compact?: boolean }) {
  ...
  return (
    <div ref={containerRef} data-cstd-case-replay={replay.id} data-cstd-worker="dedicated" data-cstd-worker-ready={workerReady ? "true" : "false"} className={compact ? "border-y border-white/15 py-6" : "border border-white/15 bg-[#07090b]/92 p-5 md:p-7"}>
      <div className="flex flex-wrap items-start justify-between gap-6">
        <div className="max-w-2xl">
          <p className="flex items-center gap-2 font-mono text-[11px] font-black text-[#24e0ff]"><Cpu aria-hidden="true" className="h-4 w-4" /> {copy.worker}</p>
        ...
        <div className="flex gap-2">
          <button type="button" onClick={run} disabled={!workerReady} className="inline-flex h-10 items-center gap-2 bg-[#f4d431] px-4 font-mono text-xs font-black text-black hover:bg-[#24e0ff] disabled:cursor-wait disabled:bg-[#485054] disabled:text-[#d7dfdc] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-[#f4d431]"><Play aria-hidden="true" className="h-3.5 w-3.5" /> {copy.run}</button>
      ...
      <label className="mt-7 block font-mono text-[11px] font-black text-[#7b878c]" htmlFor={`replay-${replay.id}-input`}>
      ...
      {result ? (
        <div className="mt-7" aria-live="polite">
          <div className="flex flex-wrap items-center justify-between gap-4 border-y border-[#3dff8f]/30 py-4 font-mono text-[11px] font-black">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {[[copy.before, result.before], [copy.after, result.after]].map(([label, values]) => (
              <div key={String(label)} className="border-l-2 border-white/20 bg-white/[0.035] px-4 py-4">
                <p className="font-mono text-[11px] font-black text-[#718087]">{String(label).toUpperCase()}</p>
          ...
          <ol className="mt-5 grid gap-2 lg:grid-cols-5">
            {result.steps.map(([label, detail], index) => (
              <li key={label} data-cstd-replay-step-active={index <= activeStep ? "true" : "false"} className="min-h-28 border border-white/12 p-3 opacity-35 transition-[opacity,border-color,transform] data-[cstd-replay-step-active=true]:-translate-y-1 data-[cstd-replay-step-active=true]:border-[#24e0ff]/55 data-[cstd-replay-step-active=true]:opacity-100">
                <p className="font-mono text-[11px] font-black text-[#24e0ff]">0{index + 1} / {label}</p>
                <p className="mt-2 text-xs leading-5 text-[#aeb7ba]">{detail}</p>
              </li>
            ))}
          </ol>
        </div>
      ) : (
        <div className="mt-7 flex items-center gap-3 border-y border-white/10 py-5 font-mono text-[11px] font-black text-[#9aa6aa]"><ArrowRight aria-hidden="true" className="h-4 w-4" /> INPUT READY / WORKER IDLE</div>
      )}
    </div>
  );
}
