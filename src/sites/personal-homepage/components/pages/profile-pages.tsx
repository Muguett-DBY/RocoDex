        ...
        <section className="border-b border-black/15 bg-[#f1eee5] px-5 py-20 text-[#0a0c0e] md:px-10 lg:px-16 lg:py-24">
          <div className="mx-auto grid max-w-[1320px] gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
            ...
            <div className="border-t border-black/20">
              {cstdProfile.education.map((entry) => <div key={entry.period} className="grid gap-4 border-b border-black/20 py-6 md:grid-cols-[8rem_14rem_1fr]"><span className="font-mono text-[11px] font-black text-black/75">{entry.period}</span><strong className="text-sm">{entry.school}</strong><p className="text-sm leading-7 text-black/65">{entry.detail[locale]}</p></div>)}
            </div>
          </div>
        </section>
    ...
    <CstdSiteChrome locale={locale} page="resume">
      <main id="cstd-main" className="cstd-resume bg-[#f1eee5] px-5 py-16 text-[#0a0c0e] md:px-10 lg:px-16 lg:py-20">
        <div className="mx-auto max-w-[1080px]">
          ...
          <section className="cstd-resume-section grid gap-8 border-b border-black/20 py-10 md:grid-cols-[13rem_1fr]"><h2 className="font-mono text-[11px] font-black text-[#0b6473]">{copy.education.toUpperCase()}</h2><div>{cstdProfile.education.map((entry) => <div key={entry.period} className="cstd-resume-entry grid gap-2 border-t border-black/15 py-5 first:border-t-0 first:pt-0 sm:grid-cols-[7rem_12rem_1fr]"><span className="font-mono text-[11px] font-black text-black/75">{entry.period}</span><strong className="text-sm">{entry.school}</strong><p className="text-sm leading-7 text-black/65">{entry.detail[locale]}</p></div>)}</div></section>
          ...
          <footer className="flex flex-wrap items-center justify-between gap-5 pt-8"><p className="font-mono text-[11px] font-black text-black/75">custard.top · Sydney / Nanjing</p><div className="flex flex-wrap gap-5"><a href="/cstd-resume.pdf" download className="inline-flex items-center gap-2 font-mono text-[11px] font-black text-[#0b6473] hover:text-black"><Download aria-hidden="true" className="h-4 w-4" />{copy.pdf}</a><CstdLink href={locale === "en" ? "/en/resume.json" : "/resume.json"} className="inline-flex items-center gap-2 font-mono text-[11px] font-black text-[#0b6473] hover:text-black"><FileJson2 aria-hidden="true" className="h-4 w-4" />{copy.json}</CstdLink><CstdLink href={localePath("/work", locale)} className="inline-flex items-center gap-2 font-mono text-[11px] font-black text-[#0b6473] hover:text-black"><FlaskConical aria-hidden="true" className="h-4 w-4" />{copy.evidence}</CstdLink></div></footer>
        </div>
      </main>
    </CstdSiteChrome>
  );
